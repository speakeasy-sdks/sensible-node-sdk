...
export class Pdf {
    ...
    async getCsvExtraction(
        ...
        const operationUrl: string = utils.generateURL(baseURL, "/generate_csv/{ids}", req);
            ...
            case httpRes?.status == 200:
                if (utils.matchContentType(contentType, `application/json`)) {
                    res.object = utils.objectToClass(
                        ...
                        operations.GetCsvExtractionResponseBody
                    );
                } else {
    ...
    async getExcelExtraction(
        ...
        const operationUrl: string = utils.generateURL(baseURL, "/generate_excel/{ids}", req);
        const client: AxiosInstance = this.sdkConfiguration.defaultClient;
        let globalSecurity = this.sdkConfiguration.security;
        if (typeof globalSecurity === "function") {
            globalSecurity = await globalSecurity();
        ...
        switch (true) {
            case httpRes?.status == 200:
                if (utils.matchContentType(contentType, `application/json`)) {
                    res.object = utils.objectToClass(
                        JSON.parse(decodedRes),
                        operations.GetExcelExtractionResponseBody
                    );
                } else {
                    throw new errors.SDKError(
                        "unknown content-type received: " + contentType,
